    ...
    public abstract class BaseApplicationContextTests : IAsyncDisposable
    {
        ...
        protected BaseApplicationContextTests()
        {
            ...
            Application = Application.WithWebHostBuilder(builder =>
            {
                ...
                {
                    services.RemoveAll(typeof(DbContextOptions<RealEstateContext>));
                    services.AddDbContext<RealEstateContext>(options =>
                    {
                        options.UseInMemoryDatabase("RealEstateDbForTesting");
                    });
                    ...
                    using (var scope = sp.CreateScope())
                    {
                        ...
                        var db = scopedServices.GetRequiredService<RealEstateContext>();
